...
findCombination (x:xs) y    | target `elem` xs = (x, target)
                            | otherwise = findCombination xs y
                            where target = y - x
findCombination [] _ = (0,0)

getFirstNumber :: [Int] -> Int
getFirstNumber (x:xs)   | findCombination xs target /= (0,0) = x
                        | otherwise = getFirstNumber xs
                        where target = 2020 - x

part1 = uncurry (*) $ findCombination inputData 2020

part2 = firstNumber * uncurry (*) (findCombination inputData (2020 - firstNumber))
        where firstNumber = getFirstNumber inputData
